  ...
  const startWatching = async () => {
    ...
    try {
      const resolveScraperSession = async () => {
        ...
        const searchRes = await fetch(`http://localhost:3001/api/scraper/search?q=${encodeURIComponent(selectedAnime.title)}`);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md transition-opacity duration-300">
            <div className="w-full h-full flex flex-col">
              <div className="flex items-center p-4 bg-[#1a1a1a]/80 border-b border-white/5">
                <button onClick={closeWatch} className="flex items-center gap-2 text-white/70 hover:text-white transition-colors">
              ...
              <div className="flex-1 flex overflow-hidden">
                ...
                <div className="w-80 bg-[#111] border-l border-white/5 overflow-y-auto hidden xl:block p-6 space-y-4">
                  ...
                  <div>
                    <h1 className="text-xl font-bold leading-tight mb-1">{selectedAnime?.title}</h1>
                    {selectedAnime?.title_japanese && (
                      <p className="text-sm text-gray-400 mb-3">{selectedAnime.title_japanese}</p>
                    )}

                    <div className="flex flex-wrap gap-2 text-xs mb-4">
                      <span className="px-2 py-1 bg-white/10 rounded">{selectedAnime?.type}</span>
                      {selectedAnime?.rating && (
                        <span className="px-2 py-1 bg-purple-900/30 text-purple-400 rounded font-medium">{selectedAnime.rating}</span>
                      )}
                  ...
                  <div className="space-y-3 text-sm border-t border-white/5 pt-4">
                    {selectedAnime?.aired?.string && (
                      <div>
                        ...
                        <p className="text-gray-300 mt-0.5">{selectedAnime.aired.string}</p>
                      </div>
                    )}
                    ...
                    {selectedAnime?.season && (
                      <div>
                        ...
                        <p className="text-gray-300 mt-0.5 capitalize">{selectedAnime.season} {selectedAnime.year}</p>
                      </div>
                    )}
                    ...
                    {selectedAnime?.duration && (
                      <div>
                        ...
                        <p className="text-gray-300 mt-0.5">{selectedAnime.duration}</p>
                      </div>
                    )}

                    <div>
                      <span className="text-gray-500 text-xs uppercase tracking-wide">Status</span>
                      <p className={`mt-0.5 ${selectedAnime?.status === 'Currently Airing' ? 'text-green-400' : 'text-gray-300'}`}>
